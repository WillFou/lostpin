...
function modeLabel(mode){ return mode==='nomove'?'No Move':mode==='nmpz'?'No Move + No Pan/Zoom':'Move'; }
...
class StatsController {
  ...
  record(game){
    ...
    const distances=game.results.map(r=>Number(r.distance)).filter(Number.isFinite);
  ...
  allPerfectRoundCount(){ return this.data.rounds.filter(r=>r.points===5000).length; }
  ...
  recentAccuracy(count){
    const rows=this.data.rounds.slice(0,count); if(!rows.length) return null;
  ...
  renderRecords(){
    ...
    for(const [key,rec] of Object.entries(this.data.records)){
      ...
      const best=Number(rec.best5)||0; if(!best && !rec.games) continue;
      rows.push({zoneId,mode,best,perfect:Number(rec.perfect5)||0,games:Number(rec.games)||0,legacy:!!rec.legacyBest,selectionName:rec.selectionName||null,selectionType:rec.selectionType||'zone'});
    }
    ...
    box.innerHTML=`<div class="statsRecordHeader"><span>Sélection / mode</span><span>Meilleur</span><span>Parties</span><span>25 000</span></div>`+rows.map(r=>{
      ...
      return `<div class="statsRecordRow"><span><b>${esc(type+zone)}</b><small>${esc(modeLabel(r.mode))}${r.legacy && !r.games?' · historique':''}</small></span><strong>${r.best?r.best.toLocaleString('fr-FR'):'—'}</strong><i>${r.games?r.games.toLocaleString('fr-FR'):'—'}</i><em>${r.perfect.toLocaleString('fr-FR')}</em></div>`;
    }).join('');
  }
