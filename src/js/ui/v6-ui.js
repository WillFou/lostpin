...
function chooseZone(zoneId) {
  ...
  // Collection UX: choose a place first, then a mode. Advanced settings remain lower down.
  scrollToSection('v6Play');
}


function updateQuickSelection() {
  const game=window.guessrGame;
  const label=$('v6QuickSelectionName');
  if (!game || !label) return;
  try { label.textContent=game.getSelectionName?.(game.getSelectionDescriptor?.()) || 'Paris'; } catch (_) {}
}
...
function bindNavigation() {
  ...
  document.querySelectorAll('[data-v6-preset]').forEach(button => {
    button.addEventListener('click', () => applyPreset(button.dataset.v6Preset,{launch:true}));
  });
