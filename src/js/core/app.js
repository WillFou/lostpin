...
class GuessrGame {
  ...
  updateRoundResultUX(points, options={}) {
    ...
    verdict.textContent = points >= 4900 ? 'Exceptionnel' : points >= 4500 ? 'Excellent' : points >= 3800 ? 'Très bien' : points >= 2800 ? 'Bien joué' : points >= 1600 ? 'Pas mal' : points > 0 ? 'Tu t’approches' : 'À retenter';
  }

  updateFinalSummary(selectionName, maxScore, options={}) {
    const valid=this.results.filter(r=>Number.isFinite(r.distance));
    const totalDistance=valid.reduce((sum,r)=>sum+Number(r.distance||0),0);
    const avgDistance=valid.length ? totalDistance/valid.length : null;
    const best=this.results.reduce((best,r,index)=>!best || Number(r.points||0)>Number(best.r.points||0)?{r,index}:best,null);
    if ($('finalBestRound')) $('finalBestRound').textContent=best ? `${Number(best.r.points||0).toLocaleString('fr-FR')} pts` : '—';
    if ($('finalBestRoundMeta')) $('finalBestRoundMeta').textContent=best ? `${options.exploration?'Mission':'Manche'} ${best.index+1} · ${best.r.description||best.r.zoneName||selectionName}` : '—';
    if ($('finalTotalLabel')) $('finalTotalLabel').textContent=options.exploration?'DISTANCE PARCOURUE':'DISTANCE TOTALE';
    if ($('finalTotalMeta')) $('finalTotalMeta').textContent=options.exploration?'sur l’ensemble des missions':'sur les réponses validées';
    if ($('finalTotalDistance')) $('finalTotalDistance').textContent=options.exploration ? formatDistance(this.results.reduce((sum,r)=>sum+Number(r.travel||0),0)) : (valid.length ? formatDistance(totalDistance) : '—');
    if ($('finalAvgLabel')) $('finalAvgLabel').textContent=options.exploration?'TEMPS MOYEN':'DISTANCE MOYENNE';
    if ($('finalAvgDistance')) $('finalAvgDistance').textContent=options.exploration ? formatClock(Math.round(this.results.reduce((sum,r)=>sum+Number(r.seconds||0),0)/Math.max(1,this.results.length))) : (avgDistance!=null ? formatDistance(avgDistance) : '—');
    if ($('finalGameMeta')) $('finalGameMeta').textContent=options.exploration ? `${selectionName} · par mission` : `${selectionName} · ${this.mode==='nomove'?'No Move':this.mode==='nmpz'?'NMPZ':'Move'} · ${variantLabel(this.gameVariant)}`;
    const ratio=maxScore>0?Math.max(0,Math.min(1,this.total/maxScore)):0;
    if ($('finalScoreBar')) requestAnimationFrame(()=>{ $('finalScoreBar').style.width=`${Math.round(ratio*100)}%`; });
  }
  ...
  finishExplorationRound() {
    ...
    this.updateRoundResultUX(points,{exploration:true});
  ...
  async startGame() {
    ...
    this.total = 0;
    this.results = [];
    if ($('finalScoreBar')) $('finalScoreBar').style.width='0%';
  ...
  async loadRound() {
    ...
    if ($('roundVerdict')) $('roundVerdict').textContent='Bien joué';
    if ($('roundProgress')) $('roundProgress').textContent=`${mission?'Mission':'Manche'} ${this.round+1} / ${this.roundCount || (mission?3:5)}`;
  ...
  endExplorationGame() {
    ...
    this.updateFinalSummary(selectionName,maxScore,{exploration:true});
