...
class MultiplayerController {
  ...
  bindUI() {
    ...
    $('leaveOnlineLobby')?.addEventListener('click', () => {
      if (!window.confirm('Quitter cette salle multijoueur ?')) return;
      this.cleanup(true); this.resetOnlineLobby();
    });
  ...
  showNotice(text, duration=2200, tone='info') {
    const stack=$('multiNoticeStack'), gameScreen=$('gameScreen');
    const inGame=!!stack && !!gameScreen && !gameScreen.classList.contains('hidden') && this.started;
    if (!inGame) { this.game.showToast?.(text,duration); return; }
    const item=document.createElement('div');
    item.className=`multiNotice multiNotice-${tone}`;
    ...
    window.setTimeout(()=>{
      ...
      window.setTimeout(()=>{ item.remove(); if (!stack.children.length) stack.classList.add('hidden'); },180);
    },Math.max(700,Number(duration)||2200));
  }

  clearNotices() {
    const stack=$('multiNoticeStack'); if (!stack) return;
    stack.replaceChildren(); stack.classList.add('hidden');
  }
  ...
  copyRoomCode() {
    if (!this.code) return;
    const copied=()=>this.showNotice(`Code ${this.code} copié.`,1800);
    if (navigator.clipboard?.writeText) navigator.clipboard.writeText(this.code).then(copied).catch(()=>this.showNotice(`Code de salle : ${this.code}`,3000));
    else this.showNotice(`Code de salle : ${this.code}`,3000);
  }
  ...
  populateOnlineSelection() {
    const select=$('onlineSelection'); if(!select) return;
    const current=this.game.getSelectionDescriptor?.() || {type:'zone',zoneId:this.game.zoneId};
    const currentName=this.game.getSelectionName?.(current) || this.api.ZONES?.[current.zoneId]?.name || 'Sélection actuelle';
    select.replaceChildren();
    const currentOption=document.createElement('option'); currentOption.value='__current__'; currentOption.textContent=`Sélection actuelle — ${currentName}`; select.appendChild(currentOption);
    Object.entries(this.api.ZONES||{}).sort((a,b)=>String(a[1]?.name||a[0]).localeCompare(String(b[1]?.name||b[0]),'fr')).forEach(([id,zone])=>{
      const option=document.createElement('option'); option.value=id; option.textContent=zone?.name||id; select.appendChild(option);
    });
    select.value='__current__';
  }

  readOnlineSelection() {
    const value=String($('onlineSelection')?.value||'__current__');
    if(value==='__current__') return this.game.getSelectionDescriptor?.() || {type:'zone',zoneId:this.game.zoneId};
    if(this.api.ZONES?.[value]) return {type:'zone',zoneId:value,name:this.api.ZONES[value].name};
    return this.game.getSelectionDescriptor?.() || {type:'zone',zoneId:this.game.zoneId};
  }

  openModal() {
    if (!this.game.apiReady) { this.showNotice('Google Maps doit être prêt avant de lancer le multijoueur.'); return; }
    this.populateOnlineSelection();
  ...
  ensureEliminationRoundCount() {
    ...
    this.showNotice(`Élimination : nombre de manches ajusté à ${adjusted}.`,2200);
    return true;
  }
  ...
  createOnlineRoom() {
    ...
    this.onlineSelection=this.readOnlineSelection();
  ...
  connectGuestToHost(isReconnect) {
    ...
    try {
      ...
      conn.on('open',()=>{
        ...
        else this.showNotice('Connexion multijoueur rétablie.',2200,'success');
      });
  ...
  handleGuestConnectionLoss(message) {
    ...
    this.showNotice('Connexion perdue · tentative de reconnexion...',2600);
  ...
  scheduleGuestReconnect() {
    ...
    if (this.reconnectAttempts>=RECONNECT_MAX_ATTEMPTS) {
      this.showNotice('Reconnexion impossible. La partie reste ouverte chez l\'hôte.',5000,'error');
      return;
    }
  ...
  handleHostData(conn,data) {
    ...
    if (data.type==='rematch:request' && this.gameEnded) {
      ...
      this.showNotice(`${player.name} demande une revanche.`,2200); return;
    }
  ...
  async startOnlineAsHost() {
    ...
    this.onlineSelection=this.onlineSelection || this.game.getSelectionDescriptor?.() || {type:'zone',zoneId:this.game.zoneId};
    this.game.roundZoneIds=this.game.buildRoundZoneSequence?.(this.onlineRoundCount,this.onlineSelection) || Array.from({length:this.onlineRoundCount},()=>this.game.zoneId);
  ...
  triggerFinalCountdown() {
    if (!this.isHost || this.roundClosed || this.onlineGameMode==='classic') return;
    ...
    this.showNotice(`Un joueur a validé : ${ONLINE_FINAL_COUNTDOWN_SECONDS} secondes restantes.`,2600);
  }

  applyRoundTimerUpdate(data) {
    if (this.kind!=='online' || !this.active || data.round!==this.round) return;
    // A Classic room keeps its original deadline even if an older host sends a first-submit countdown packet.
    if (data.reason==='first-submit' && this.onlineGameMode==='classic') return;
    const seconds=Math.max(0,Number(data.seconds)||0);
    if (!seconds) return;
    if (Number.isFinite(Number(data.deadline)) && Number(data.deadline)>Date.now()) this.startRoundTimer(Math.ceil((Number(data.deadline)-Date.now())/1000),Number(data.deadline));
    else this.startRoundTimer(seconds);
    if (data.reason==='first-submit') this.showNotice(`Un joueur a validé : ${seconds} secondes restantes.`,2600);
  }
  ...
  renderOnlineStatus(data) {
    ...
    if (firstNew && previous.size>0 && firstNew.id!==this.myPlayer()?.id) this.showNotice(`${firstNew.name} a validé sa position.`,1700);
  }
  ...
  returnToOnlineLobbyForRematch() {
    ...
    $('gameScreen')?.classList.add('hidden'); this.hideMultiEnd(); $('startScreen')?.classList.remove('hidden');
  ...
  enterRematchLobby(data) {
    ...
    $('gameScreen')?.classList.add('hidden'); this.hideMultiEnd(); $('startScreen')?.classList.remove('hidden');
    $('multiplayerModal')?.classList.remove('hidden'); this.showOnlineLobby(); this.renderOnlineLobby();
    this.showNotice("L'hôte propose une revanche. Indique quand tu es prêt.",3200);
  }
  ...
  fatalMulti(error) { console.error(error); this.showNotice(error.message||'Erreur multijoueur',5000,'error'); this.cleanup(false); this.originalShowMenu(); }
